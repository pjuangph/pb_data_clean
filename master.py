    ...
    for i in range(len(workbooks_to_combine)):
        ...
        for i in range(len(df_sheet_clean)):
            ...
            if len(temp)>0: # Data exists
                ...
                try:
                    newWard = int(df_sheet_clean.iloc[i]["I live in Cleveland Ward # "])
                except:
                    newWard = -1
                if df_master_clean.iloc[j]["Ward"] != df_sheet_clean.iloc[i]["I live in Cleveland Ward # "] and newWard>0:
        ...
        pd.concat([df_master_clean,pd.DataFrame(data_to_add)],ignore_index=False).to_excel('master_clean.xlsx','PB CLE Master List')
